...
| 12 | Implement TZ v3 Iteration 1 & 2 | Feature | Completed | `Navbar.tsx`, `SlideDeck.tsx`, `ModuleTimeline.tsx`, `ResourceLibrary.tsx`, `types.ts` | G1, G2, G3 | Reduced student nav noise, added slides CTA, week filter for resources, new artifact types. |
| 13 | Finalize TZ v3 Iterations 9-10 & TS/Lint Fixes | Fix | Completed | `courseData.ts`, `PromptBuilder.tsx`, `types.ts`, `App.tsx`, `CodeEditor.tsx`, `TeacherDashboard.tsx`, `supabaseClient.ts` | G2, G3, G4 | Safely fixed all TS compile errors (missing type fields, any overrides, optional properties). Verified passing `npm run build` and `npm run lint`. |
